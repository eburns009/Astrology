...
def calculate_lahiri_ayanamsa(jd: float) -> float:
    ...
    # More accurate formula used by Indian Astronomical Ephemeris
    T = (jd - 2415020.0) / 365.25  # Years from 1900.0
    
    # Official Lahiri formula (Government of India)
    # Value at 1900.0: 22°27'38.48" 
    base_1900 = 22.460689  # degrees
    
    # Current rate: 50.2388475"/year (this is the key - different from Fagan-Bradley)
    annual_rate = 50.2388475 / 3600.0  # Convert to degrees
    
    # Calculate ayanamsa for the date
    ay = base_1900 + (T * annual_rate)
...
def chart():
    ...
    try:
        ...
        if zodiac in ["sidereal", "sidereal_fb"]:
            ayanamsa_type = "fagan-bradley" if zodiac == "sidereal_fb" else "lahiri"
            ayanamsa = calculate_ayanamsa(dt, ayanamsa_type)
            longitudes = {name: normalize_deg(lon - ayanamsa) for name, lon in longs_trop.items()}
            zodiac_display = f"Sidereal ({ayanamsa_type.title()})"
        else:
            ayanamsa = 0.0
            longitudes = longs_trop.copy()
            zodiac_display = "Tropical"
